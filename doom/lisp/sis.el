...
(defvar sis-inline-with-english nil
...
(defvar sis--insert-remembered nil
  "Input source remembered from the last insert mode session.")
(make-variable-buffer-local 'sis--insert-remembered)

(defun sis--remember-before-insert-exit ()
  "Save current input source before switching to english on insert exit."
  (setq sis--insert-remembered sis--current))

(defun sis--restore-on-insert-entry ()
  "Restore remembered input source when entering insert mode."
  (when sis--insert-remembered
    (sis--set sis--insert-remembered)))
...
(defun sis--respect-evil ()
  "Respect evil."
  (when (featurep 'evil)
    (add-hook 'evil-insert-state-exit-hook #'sis--remember-before-insert-exit -10)
    (add-hook 'evil-insert-state-exit-hook #'sis-set-english)
    (add-hook 'evil-insert-state-entry-hook #'sis--restore-on-insert-entry)
...
- Respect buffer: restore buffer input source when it regain focus."
  ...
  (cond
   ...
   ((not sis-global-respect-mode)
    ...
    ;; for evil
    (when (featurep 'evil)
      (remove-hook 'evil-insert-state-exit-hook #'sis--remember-before-insert-exit)
      (remove-hook 'evil-insert-state-exit-hook #'sis-set-english)
      (remove-hook 'evil-insert-state-entry-hook #'sis--restore-on-insert-entry)
